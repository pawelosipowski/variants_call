...
threads=X           Spawn exactly X hashing threads (default is number of logical processors).  Total active threads may exceed X due to I/O threads.
...
target=40           (tgt) Target normalization depth.  NOTE:  All depth parameters control kmer depth, not read depth.
...
mindepth=6          (min) Kmers with depth below this number will not be included when calculating the depth of a read.
